...
function saveDraft() {
    const formData = {
        ...
        finishingMaterial: document.getElementById('finishingMaterial').value,
        windowsType: document.getElementById('windowsType').value,
        heatingType: document.getElementById('heatingType').value,
        sewageType: document.getElementById('sewageType').value,
        constructionTime: document.getElementById('constructionTime').value,
        paymentMethod: document.getElementById('paymentMethod').value,
        services: []
    };
    
    document.querySelectorAll('.maric-checkbox-group input[type="checkbox"]:checked').forEach(checkbox => {
        formData.services.push(checkbox.id);
    });
    
    localStorage.setItem('constructionDraft', JSON.stringify(formData));
    alert('Черновик сохранен! Вы можете продолжить расчет позже.');
}

function loadDraft() {
    const draft = JSON.parse(localStorage.getItem('constructionDraft'));
    if (draft) {
        document.getElementById('area').value = draft.area;
        document.getElementById('floors').value = draft.floors;
        document.getElementById('houseType').value = draft.houseType;
        document.getElementById('roofType').value = draft.roofType;
        document.getElementById('wallMaterial').value = draft.wallMaterial;
        document.getElementById('foundationType').value = draft.foundationType;
        document.getElementById('finishingMaterial').value = draft.finishingMaterial;
        document.getElementById('windowsType').value = draft.windowsType;
        document.getElementById('heatingType').value = draft.heatingType;
        document.getElementById('sewageType').value = draft.sewageType;
        document.getElementById('constructionTime').value = draft.constructionTime;
        document.getElementById('paymentMethod').value = draft.paymentMethod;
        
        draft.services.forEach(serviceId => {
            const checkbox = document.getElementById(serviceId);
            if (checkbox) checkbox.checked = true;
        });
        
        alert('Черновик загружен! Продолжите расчет.');
    } else {
...
function sendByEmail() {
    alert('Функция отправки по email будет реализована позже');
}
